...
|-------|----------|----------------|
| **A** | **Signatures + Band Index** | `odin_sig` + Python `sqlite3` (zero external dependencies) |
...
2. **Signature generation** (fast, derived from embedding) — 0din-sig SDK
...
| Aspect | 0din-sig | Annoy | Faiss LSH |
|--------|----------|-------|-----------|
...
**When to use 0din-sig**: Need deterministic signatures, cross-language consistency, or air-gapped deployment.
